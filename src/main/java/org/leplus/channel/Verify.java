...
public final class Verify implements ActionListener {
  ...
  public void actionPerformed(final ActionEvent ae) {
    try (ObjectInputStream pubStream =
            new ObjectInputStream(new FileInputStream(pubField.getText()));
        FileInputStream fileStream = new FileInputStream(fileField.getText());
        ObjectInputStream sigStream =
            new ObjectInputStream(new FileInputStream(sigField.getText()))) {
      ...
      if (b) {
        ...
        final BigInteger X =
            d.digest()
                .getInt()
                .multiply(sig.getS().subtract(pk.getY()).modInverse(pk.getQ()))
                .mod(pk.getQ());
  ...
  private String split(final String string) {
    ...
    while (i <= string.length() - 50) {
      result
          .append("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;")
          .append(string.substring(i, i + 49))
          .append("<br>");
      i += 50;
    }
    result
